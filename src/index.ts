...
import {
	ModalWithBusket,
	ModalWithContactsForm,
	ModalWithItem,
	ModalWithOrderComplete,
	ModalWithOrderForm,
...
const cardCatalogItem = document.getElementById(
	'card-catalog'
...
const modalWithBasket = new ModalWithBusket(
	baseModal,
	events,
	document.getElementById('basket') as HTMLTemplateElement
);

const modalWithOrder = new ModalWithOrderForm(
	baseModal,
	events,
	document.getElementById('order') as HTMLTemplateElement
);

const modalWithContacts = new ModalWithContactsForm(
	baseModal,
	events,
	document.getElementById('contacts') as HTMLTemplateElement
);

const modalWithOrderComplete = new ModalWithOrderComplete(
	baseModal,
	events,
...
events.on('basket:open', () => {
	const cardsArr = cardsData.getAddedCards().map((card) => {
		const cardInstant = new CardInBasket(
...
events.on('orderContacts:submit', (data: { email: string; phone: string }) => {
	...
	orderData.setOrderInfo({
		...
		items: cardsData.getAddedCardsList(),
	});
	api.orderProducts(orderData.getOrderInfo()).then((data: IOrderResult) => {
